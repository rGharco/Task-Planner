    ...
    useEffect(() => {
        async function loadHistory() {
            try {
                ...
                const response = await fetch(`https://task-planner-api-bmko.onrender.com/api/tasks/userHistory?userId=${targetUserId}`); 
    ...
    useEffect(() => {
        if (userCookie.role === "manager") {
            async function loadSubordinates() {
                try {
                    const response = await fetch(`https://task-planner-api-bmko.onrender.com/api/users/getManagerExecutors?managerId=${userCookie.id}`); 
